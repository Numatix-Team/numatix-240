...
class Strategy:
    ...
    def __init__(self, broker, config_path: str = "config.json") -> None:
        self.broker = broker
...
class StrategyBroker:
    def _init_(self, config_path="config.json"):
    ...
    def current_price(self, symbol, exchange):
        ...
        if k not in self.subs:
            c = self.ib.create_contract(symbol, "IND", exchange, "USD")
            self.subs[k] = self.ib.subscribe_market_data(c)
            return None
        return self.ib.get_live_data(self.subs[k])

    def get_option_tick(self, symbol, expiry, strike, right):
        k = f"{symbol}-{expiry}-{strike}-{right}"
        if k not in self.subs:
            c = self.ib.create_contract(symbol, "OPT", "SMART", "USD", expiry, strike, right)
            self.subs[k] = self.ib.subscribe_market_data(c)
            return None
        return self.ib.get_live_data(self.subs[k])

    def get_option_ohlc(self, symbol, expiry, strike, right, duration="1 D", bar="1 min"):
        c = self.ib.create_contract(symbol, "OPT", "SMART", "USD", expiry, strike, right)
        bars = self.ib.fetch_historical_data(c, duration, bar)
        return pd.DataFrame([vars(b) for b in bars]) if bars else pd.DataFrame()
    ...
    def place_option_market_order(self, symbol, expiry, strike, right, qty, action):
        ...
        o.orderType = "MKT"
        o.totalQuantity = qty
        return self.ib.place_order(c, o)
